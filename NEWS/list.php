<?php
	...
	$sql2="select * from cms_news where status=1 and menuid=$menuid";
	$res2=mysqli_query($link,$sql2);
	$totalData=mysqli_num_rows($res2);
	//page
	$totalpage=mysqli_num_rows($res2);
	$pagelistT=4;
		...
		<div class="left">
			...
			<?php if($totalData>0) {?>
				...
				<div class="pic-txt">
					...
					<p>
					  <span><?=$rowPage['comefrom']?></span><span><?=$rowPage['count']?>评</span><span><?php $a=($rowPage['create_time']/3600);$b=(time()/3600);echo round($b-$a,2);?>小时前</span>
					</p>
					<span style="position: absolute;top:109%;left: 40%;"><?=pagelist()?></span>
				</div>
				<?php }?>
				<?php }else{
					echo "<span style='font-size:50px;display:block;width:100%; text-align:center;float:none;'>没有数据</span>";
				}?>
			</div>
